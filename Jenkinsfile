pipeline {
  ...
  script {
            sh """
                # Build with BUILD_NUMBER as tag
                docker build -t ${DOCKER_HUB_USER}/${DOCKER_HUB_REPO}:${BUILD_NUMBER} .

                # Also create latest tag
                docker tag ${DOCKER_HUB_USER}/${DOCKER_HUB_REPO}:${BUILD_NUMBER} ${DOCKER_HUB_USER}/${DOCKER_HUB_REPO}:latest
            """
        }
